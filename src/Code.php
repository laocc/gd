...
final class Code extends BaseGD
{
    ...
    private function createCode1(&$img, &$code, array $opt)
    {
        ...
        imagefilledrectangle($img,
            ...
            $this->createColor($img, $opt['b_color']));//画矩形并填充背景色

        //生成验证码
        if (!isset($opt['code']) or !$opt['code']) {
            $_code_len = is_int($opt['length']) ? $opt['length'] : mt_rand(...$opt['length']);
            $code = array_rand(array_flip(str_split($this->disc[$opt['charset']], $cn ? 3 : 1)), $_code_len);
        } else {
        ...
        for ($i = 0; $i < (is_int($opt['line']) ? $opt['line'] : mt_rand(...$opt['line'])); $i++) {
            imageline($img,
                ...
                $this->createColor($img, $opt['l_color']));   //颜色
        }
